...
const t=parseRanking(html,'https://chess-results.com/tnr123.aspx?art=1','U8');assert.equal(t.players[0].id,'123-4');assert.equal(t.players[0].points,1.5);assert.deepEqual(t.tieLabels,['Hệ số 1']);
...
const playerCase1 = {
  ...
  rounds: [
    ...
    { round: 6, board: 5, opponent: 'Opp 6', color: 'BLACK' as const, score: null, status: 'scheduled' as const, result: '—' }
  ]
};
...
assert.equal(nextMatchCase1?.board, 5);
